...
/obj/pod
	...
	proc/TakeDamage(var/damage = 0, var/ignore_defense = 0, var/obj/item/I, var/mob/living/attacker = 0)
		...
		var/obj/item/weapon/pod_attachment/armor/armor = GetAttachmentOnHardpoint(P_HARDPOINT_ARMOR)
		var/obj/item/weapon/pod_attachment/shield/shield = GetAttachmentOnHardpoint(P_HARDPOINT_SHIELD)
		...
		var/datum/effect_system/spark_spread/hit_sparks = new()
	...
	proc/DestroyPod()
		...
		for(var/mob/living/carbon/human/H in GetOccupants())
			...
			if(istype(get_turf(src), /turf/space) && inertial_direction)
	...
	blob_act(var/severity)
		TakeDamage(pod_config.blob_act_damage * (1 / severity))
