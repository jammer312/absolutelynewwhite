...
/obj/machinery/spod_part_fabricator/proc/build_part(datum/design/D)
	...
	visible_message(" <b>\The [src]</b> beeps, \"\The [I] is complete.\"")
	being_built = null
...
/obj/machinery/spod_part_fabricator/proc/process_queue()
	...
	while(D)
		...
		if(!check_resources(D))
			visible_message(" <b>\The [src]</b> beeps, \"Not enough resources. Queue processing stopped.\"")
	...
	visible_message(" <b>\The [src]</b> beeps, \"Queue processing finished successfully.\"")
...
/obj/machinery/spod_part_fabricator/proc/sync()
	...
	for(var/obj/machinery/computer/rdconsole/RDC in localarea.contents)
		...
		updateUsrDialog()
		visible_message(" <b>\The [src]</b> beeps, \"Successfully synchronized with R&D server.\"")
		return
...
/obj/machinery/spod_part_fabricator/interact(mob/user as mob)
	...
	if(exit.density)
		visible_message(" <b>\The [src]</b> beeps, \"Error! Part outlet is obstructed.\"")
		return
	if(temp)
		left_part = temp
	else if(being_built)
...
/obj/machinery/spod_part_fabricator/attackby(obj/W as obj, mob/user as mob, params)
	...
	if(panel_open)
		if(istype(W, /obj/item/weapon/crowbar))
			...
			user.visible_message(user, "<span class='danger'>You can't load \the [name] while it's opened.</span>")
			return 1
	...
	if(istype(W, /obj/item/stack))
		...
		if(being_built)
			user.visible_message(user, "\The [src] is currently processing. Please wait until completion.")
			return
		if(res_max_amount - resources[material] < MINERAL_MATERIAL_AMOUNT) //overstuffing the fabricator
			user.visible_message(user, "\The [src] [material2name(material)] storage is full.")
			return
		...
		if(resources[material] < res_max_amount)
			...
			user.visible_message(user, "You insert [transfer_amount] [sname] sheet\s into \the [src].")
			sleep(10)
			updateUsrDialog()
			overlays -= "fab-load-[material2name(material)]" //No matter what the overlay shall still be deleted
		else
			user.visible_message(user, "\The [src] cannot hold any more [sname] sheet\s.")
		return
