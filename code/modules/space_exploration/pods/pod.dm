...
/obj/pod
	...
	proc/DoMove(var/mob/user, var/_dir)
		...
		if(!HasPower(movement_cost)) //åòî êîñòûëü, òóò èçíà÷àëüíî áûëî if(!HasPower(pod_config.movement_cost)), íî îíî íå ðàáîòàëî
...
//			return 0 //Ðàçíåðô
		...
		if(user != pilot) /*(istype(get_turf(src), /turf/space) && !HasTraction())*/ //ÂÎÒ ÝÒÎ ÂÎÒ ÄÅËÀÅÒ ÑÎÂÑÅÌ ÍÀÎÁÎÐÎÒ ÍÅ ÒÎ ×ÒÎ ÍÀÄÎ
