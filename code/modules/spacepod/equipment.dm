/obj/item/device/spacepod_equipment/weaponry/proc/fire_weapons()
	if(my_atom.next_firetime > world.time)
		user.visible_message(usr, "<span class='warning'>Your weapons are recharging.</span>")
		return
	var/turf/firstloc
	var/turf/secondloc
	if(!my_atom.equipment_system || !my_atom.equipment_system.weapon_system)
		user.visible_message(usr, "<span class='warning'>Missing equipment or weapons.</span>")
...
/obj/item/device/spacepod_equipment/lock/keyed/attackby(obj/item/I as obj, mob/user as mob, params)
	if(istype(I, /obj/item/device/spacepod_key))
		...
		if(!key.id)
			key.id = id
			user.visible_message(user, "<span class='notice'>You grind the blank key to fit the lock.</span>")
		else
			user.visible_message(user, "<span class='warning'>This key is already ground!</span>")
	else
		..()
